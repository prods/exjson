...
def get_week(date: datetime.date):
    return date.strftime('%w').zfill('0', 2)


_DATETIME_FORMAT_MAP = [
    (0, "dddd", "%A", None),  # Weekday as locale’s full name. Ex. "Monday"
    (1, "ddd", "%a", None),  # Weekday as locale’s abbreviated name. Ex. "Mon"
    (2, "w", "%w", None), # Weekday as a decimal number, where 0 is Sunday and 6 is Saturday. Ex. "1"
    (3, "ww", "", get_week), # Weekday as zero padded number
    (3, "dd", "%d", None),  # Day of the month as a zero-padded decimal number. Ex. "30"
    (4, "d", "%-d", None),  # Day of the month as a decimal number. (Platform specific) Ex. "30"
    (5, "MMMM", "%B", None),  # Month as locale’s full name. Ex. "September"
    (6, "MMM", "%b", None),  # Month as locale’s abbreviated name. Ex. "Sep"
    (7, "MM", "%m", None),  # Month as a zero-padded decimal number. Ex. "09"
    (8, "M", "%-m", None),  # Month as a decimal number. (Platform specific) Ex. "9"
    (9,"yyyy", "%Y", None),  # Year with century as a decimal number. Ex. "2013"
    (10, "y", "%y", None),  # Year without century as a zero-padded decimal number. Ex. "13"
    (11, "HH", "%H", None),  # Hour (24-hour clock) as a zero-padded decimal number. Ex. "07"
    (12, "H", "%-H", None),  # Hour (24-hour clock) as a decimal number. (Platform specific) Ex. "7"
    (13, "hh", "%I", None),  # Hour (12-hour clock) as a zero-padded decimal number. Ex. "07"
    (14, "h", "%-I", None),  # Hour (12-hour clock) as a decimal number. (Platform specific) Ex. "7"
    (15, "tt", "%p", None),  # Locale’s equivalent of either AM or PM. Ex. "AM"
    (16, "mm", "%M", None),  # Minute as a zero-padded decimal number. Ex. "06"
    (17, "m", "%-M", None),  # Minute as a decimal number. (Platform specific) Ex. "6"
    (18, "ss", "%S", None),  # Second as a zero-padded decimal number. Ex. "05"
    (19, "s", "%-S", None),  # Second as a decimal number. (Platform specific) Ex. "5"
    (20, "f", "%f", None),  # Microsecond as a decimal number, zero-padded on the left. Ex. "000000"
    (21, "zzz", "%Z", None),  # Time zone name (empty string if the object is naive). Ex. ""
    (22, "z", "%z", None),  # UTC offset in the form +HHMM or -HHMM (empty string if the the object is naive). Ex. ""
    #(23, "y": "%j",  # Day of the year as a zero-padded decimal number. Ex. "273"
    ...
    (27, "F", "%c", None),  # Locale’s appropriate date and time representation. Ex. "Mon Sep 30 07:06:05 2013"
    (28, "D", "%x", None),  # Locale’s appropriate date representation. Ex. "09/30/13"
    (29, "T", "%X", None),  # Locale’s appropriate time representation. Ex. "07:06:05"
    (30, "Z", "%Y-%m-%dT%H:%m:%s.%f%z", None)  # Full ISO-8601
]
